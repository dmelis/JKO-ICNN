# -*- coding: utf-8 -*-
"""Copy of convex_potential_flow MNIST.ipynb
# Simple ICNN on toy data
"""
...
dataset = 'MAFMoon'   # ['SwissRoll', 'TwoSpirals', 'EightGaussian', 'Rings', 'MAFMoon']
img_file = ''  # ''imgs/twitter.png'   # replace this if want to generate 2d image file and comment out `dataset`
dimx = 2
nblocks = 5
depth = 5
k = 256
...
plt.axis('off')
plt.tight_layout()
plt.savefig(f'{ToyData.__name__}_data.png')


if flow_type == 'cpflow':

    icnns = [ICNN3(dimx, k, depth, symm_act_first=False, softplus_type='softplus',
...
for e in range(num_epochs):
    for x in train_loader:
        ...
        if clip_grad == 0:
            ...
            grad_norm = torch.nn.utils.clip_grad.clip_grad_norm_(flow.parameters(), clip_grad)
...
fig = plt.figure(figsize=(5, 5))
plot_logp(4, 200, cmap=cmap)
plt.tight_layout()
plt.savefig(f'{ToyData.__name__}_{nblocks}_{flow_type}_{depth}_{k}.png')
...
# for plotting the potential of cpflow (if using 1 block)
if nblocks == 1 and flow_type == 'cpflow':
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig(f'{ToyData.__name__}_{nblocks}_{depth}_{k}_contour.png')

    plt.figure(figsize=(5, 5))
    plt.contourf(f.reshape(n, n), levels=20)
    plt.axis('off')
    plt.tight_layout()
    plt.savefig(f'{ToyData.__name__}_{nblocks}_{depth}_{k}_contourf.png')
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig(f'{ToyData.__name__}_{nblocks}_{depth}_{k}_grad.png')
    ...
    plt.hlines(-b, -b, b, color='red')
    plt.savefig(f'{ToyData.__name__}_{nblocks}_{depth}_{k}_z.png')
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig(f'{ToyData.__name__}_{nblocks}_{depth}_{k}_x_meshgrid.png')
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig(f'{ToyData.__name__}_{nblocks}_{depth}_{k}_z_meshgrid.png')
    ...
    if num_samples:
        ...
        plt.axis('off')
        plt.tight_layout()
        plt.savefig(f'{ToyData.__name__}_{nblocks}_{depth}_{k}_z_sample.png')

        plt.figure(figsize=(5, 5))
        plt.scatter(x[:, 0], -x[:, 1], s=3, color=colors)
        plt.axis('off')
        plt.tight_layout()
        plt.savefig(f'{ToyData.__name__}_{nblocks}_{depth}_{k}_x_sample.png')
        ...
        # ani.save(f'{ToyData.__name__}_{nblocks}_{depth}_{k}_x_sample.mp4', writer=writer)
        ani.save(f'{ToyData.__name__}_{nblocks}_{depth}_{k}_x_sample.gif', writer=writer)
    ...
    if ToyData.__name__ == 'EightGaussian':
        ...
        plt.axis('off')
        plt.tight_layout()
        plt.savefig(f'{ToyData.__name__}_{nblocks}_{depth}_{k}_z_encode.png')
        ...
        plt.axis('off')
        plt.tight_layout()
        plt.savefig(f'{ToyData.__name__}_{nblocks}_{depth}_{k}_x_raw.png')
