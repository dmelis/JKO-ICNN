...
### TAKEN FROM https://pysgmcmc.readthedocs.io/en/pytorch/_modules/pysgmcmc/optimizers/sgld.html
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for parameter in group["params"]:
                ...
                ### HACKY: MY INTERPRETATION OF SGLD
                temp = group["temp"]
                β = 1/temp
                scaled_grad = gradient + torch.normal(
                    mean=torch.zeros_like(gradient),
                    std=torch.ones_like(gradient)
                ) *  torch.sqrt(  2  / (β*torch.tensor(lr)))
